...
public abstract class CallActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().addFlags(
				WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON | WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED | WindowManager.LayoutParams.FLAG_TURN_SCREEN_ON);
	...
	protected abstract void setAudioDevice(AudioDevice currentAudioDevice,
	...
	private void init() {
		...
		Log.i(TAG,
				"[CallActivity] init() => (mState: " + mState + ", mCallId: " + mCallId + ", " +
						"mIsVideoCall: " + mIsVideoCall + ", mCalleeIdToDial: " + mCalleeIdToDial + ", mDoDial: " + mDoDial + ", mDoAccept: " + mDoAccept + ", mDoLocalVideoStart: " + mDoLocalVideoStart + ", mDoEnd: " + mDoEnd + ")");
	...
	protected boolean setState(STATE state, DirectCall call) {
		...
		switch (state) {
			case STATE_ACCEPTING: {
				...
				mImageViewEnd.setBackgroundResource(R.drawable.call_hangup);
	...
	private void bindCallService() {
		...
		bindService(new Intent(this, SendbirdCallService.class), mCallServiceConnection,
