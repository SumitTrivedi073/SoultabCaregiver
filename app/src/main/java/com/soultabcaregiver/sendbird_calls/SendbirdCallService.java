...
public class SendbirdCallService extends Service {
	...
	private Notification getNotification(@NonNull ServiceData serviceData) {
		...
		builder.setContentTitle(serviceData.remoteNicknameOrUserId)
				...
				.setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(), R.drawable.main_logo))
	...
	public static void dial(Context context, String doDialWithCalleeId, String name, boolean isVideoCall) {
		if (SendBirdCall.getOngoingCallCount() > 0) {
			Utility.ShowToast(context, "Ringing.");
			Log.i(TAG, "[CallService] dial() => SendBirdCall.getOngoingCallCount(): " + SendBirdCall.getOngoingCallCount());
			return;
		}
	...
	public void updateNotification(@NonNull ServiceData serviceData) {
		Log.i(TAG, "[CallService] updateNotification(isHeadsUpNotification: " + serviceData.isHeadsUpNotification + ", remoteNicknameOrUserId: " + serviceData.remoteNicknameOrUserId
