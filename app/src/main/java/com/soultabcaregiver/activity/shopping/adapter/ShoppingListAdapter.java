    ...
    /*public void UpdateUserProfile(String webUrl) {
        final VolleyMultipartRequest multipartRequest =
                new VolleyMultipartRequest(Request.Method.POST,
                        APIS.BASEURL + APIS.UPDATEPROFILEAPI,
                        new Response.Listener<NetworkResponse>()////Place user for service
                        {
                            @Override
                            public void onResponse(NetworkResponse response) {
                                ...
                                try {
                                    ...
                                    if (profileModel.getStatus().equalsIgnoreCase("true")) {
                                        ...
                                        APIS.setSharedPreference(mcontext,
                                                APIS.is_40plus_user, "1");
                                        Intent intent =
                                                new Intent(mcontext, ActSocialActivity.class);
                                        intent.putExtra("webUrl",
		                                        webUrl + "?userId =" + APIS.getSharedPreferences(
				                                        mcontext, APIS.is_40plus_userID));
                                        intent.putExtra("title", mcontext.getResources().getString(
                                                R.string.plusmart));
                                        intent.putExtra(IntentKey.FromWhere,
                                                IntentKey.isShoppingcategory);
                                        mcontext.startActivity(intent);
                    ...
                    protected Map<String, String> getParams() {
                        ...
                        params.put("user_id", sessionParam.user_id);
                        params.put("name",
                                APIS.getSharedPreferences(mcontext, APIS.name));
                        params.put("lastname",
                                APIS.getSharedPreferences(mcontext, APIS.lastname));
                        params.put("mobile_no",
                                APIS.getSharedPreferences(mcontext, APIS.mobile));
                        params.put("countrycode",
                                APIS.getSharedPreferences(mcontext, APIS.country_code));
                        params.put("is_40plus_user", "1");
